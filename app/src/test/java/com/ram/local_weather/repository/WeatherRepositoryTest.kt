...
class WeatherRepositoryTest {
    ...
    @Test
    fun getWeatherForecastDataMockResponse() = runTest{
        ...
        Mockito.`when`(weatherService.getWeatherDataFromLocation(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(Response.success(queryWeatherResponse))
        val result = weatherRepository.getWeatherDataFromLocation("pune")
        Assert.assertEquals("Pune", result.data?.name)
    }

    @Test
    fun getWeatherForecastWrongCityNameTest() = runTest {
        Mockito.`when`(weatherService.getWeatherDataFromLocation(ArgumentMatchers.anyString(),ArgumentMatchers.anyString(),ArgumentMatchers.anyString())).thenReturn(Response.error(404, "city not found".toResponseBody()))
    ...
    @Test
    fun getWeatherDataMockRespone() = runTest {
    ...
    fun getWeatherData() = runTest {
        Mockito.`when`(weatherService.getWeatherData(any(), any(), any(), any())).thenReturn(
